...
## DEC-022: Hand-rolled utility AI mirroring `bevy_observed_utility` patterns (Phase 5)
- **Date**: 2026-04-30
- **Status**: Accepted (supersedes the Phase 5 spec note's DEC-034 framing)
- **Context**: Phase 5 spec §7.9 needs utility AI -- many independent scorers, highest wins per tick, hysteresis to prevent thrashing. Original Phase 5 framing said "hand-roll because `big-brain` is archived." That premise is wrong: the GitHub repo was archived 2025-10-07 but development continues on Codeberg (currently at Bevy 0.16/0.17). The premise that's actually load-bearing in 2026-04 is the Bevy version gap: this game is on Bevy 0.18, and *no* utility AI crate is on 0.18 yet. `big-brain` (Codeberg) sits at 0.17. `bevy_observed_utility` (modern, ECS-observer-based, architecturally the cleanest fit) sits at 0.15. `bevy_behave` is the only AI crate currently on 0.18, but it's behavior trees, not utility AI -- adopting it would mean rewriting §7.9 around a different paradigm
- **Decision**: Hand-roll a small utility AI core (~150 LOC) under `src/ai/`. Mirror `bevy_observed_utility`'s shape: scorers as child entities of the actor, a `Picker` component on the actor with named strategies (`Highest`, `FirstToScore`, `Random`), an `ActiveAction` component holding the currently-running action, and lifecycle observers for action transitions. Hysteresis lives in the picker as a configurable margin (default 0.05). Scorer evaluation is a registry of `fn(&World, Entity) -> f32`; action execution is one Bevy system per `ActionKind`, run-conditioned on the active action. Adding a new scorer or action is a one-file change
- **Alternatives**: (a) Adopt `big-brain` from Codeberg and port from 0.17 to 0.18 ourselves -- non-trivial port, plus we'd own the fork forever or wait for upstream. (b) Adopt `bevy_observed_utility` and port from 0.15 to 0.18 -- bigger version gap, same forking problem. (c) Switch to behavior trees via `bevy_behave` -- would compile against 0.18 but is the wrong paradigm for "score N options, pick best per tick" patterns like SitInSunbeam vs. VisitFriend. (d) Adopt `bevy_dogoap` (GOAP) -- wrong tool; cats don't plan multi-step action sequences toward a goal, they react to current world state
- **Consequences**: Zero abandonment risk on a system that will see continuous extension across Phases 5-6. ~150 LOC stays in-tree; we maintain it. Picking `bevy_observed_utility`'s shape (rather than inventing one) means the migration path is short if its 0.18 release lands -- swap our types for theirs, keep our scorer/action functions. DEBT-022 tracks this. Phase 5 spec note "DEC-034" is folded into this entry; the spec text under W5.7 stays accurate for behavior, only the rationale changes
...
## DEC-023: Drive kitten gait animation from measured velocity, not input intent
- **Date**: 2026-05-02
- **Status**: Accepted
- **Context**: Phase 5 wired the player kitten to play Mixamo Idle/Walk/Run/Jump/Pickup/Swim clips via `drive_kitten_animation`. The first cut chose the gait from `iso_movement().length_squared() > 0` plus `Action::Sprint pressed`. Playtesting indoors surfaced "I can't run but the running animation plays" — input was forward + Sprint while the cat was walled in, so the run cycle looped forever while the cat stood still. The bug was structural: animation read intent, not state
- **Decision**: Locomotion gait selection (idle / walk / run) reads horizontal velocity from `bevy_rapier3d::Velocity` on the player and gates by speed thresholds (`WALK_GATE` 0.4 m/s, `RUN_GATE` 6.0 m/s — between the 5 m/s walk cap and 8 m/s run cap). Jump still gates on `TnuaController::is_airborne` (state from physics). Swim gates on player Y vs water surface — also state. Pickup latches on `GatherEvent` for `PICKUP_DURATION` and plays one-shot. **Input intent never directly drives a continuous animation.** One-shot animations (pickup, future attack) keep reading events because that's the only signal that exists. `Velocity::default()` is added to the player spawn bundle so rapier writes velocity into ECS — without that explicit insertion the component query returns nothing
- **Alternatives**: (a) Gate by input intent + `&& movement_blocked` flag — duplicates collision detection in the animation system. (b) Use `Transform` delta over `Time::delta_secs` — works without rapier's Velocity component but loses precision around frame-rate jitter and collision corrections. (c) Read `TnuaController`'s desired velocity rather than rapier's actual velocity — defeats the point: the bug was that desired and actual diverged. (d) Drive ALL animations from velocity / state and refuse to expose input — too far; one-shot triggers like pickup need an event signal
- **Consequences**: Cat's animation always reflects what it's *doing*, never what it's *trying* to do. Same principle is the obvious fit for NPC cats once Phase 5 lands the AI dispatcher (`big-brain`-shaped scorers can publish "want to wander" intent, but the visible gait will read off the rigid body). Per-gait playback rate multipliers (`WALK_ANIM_SPEED` 2.0, `RUN_ANIM_SPEED` 2.3, `PICKUP_ANIM_SPEED` 3.0) are kept as constants near the top of `src/player/mod.rs` rather than computed from velocity, since the Mixamo clips' natural pace is fixed — only the player's expected speed matters and that's already a constant (`RUN_SPEED`). Sneak is still loaded but unused — reserved for a future stealth input which (when it lands) will gate intent-style. The pattern is recorded as a feedback memory (`feedback_animation_measured_velocity.md`)
